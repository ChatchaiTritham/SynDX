...
class ComparativeAcademicCharts:
    ...
    def fig8_comparative_performance(self, explorer, param_space):
        ...
        ax1.set_title('(A) Multi-Metric Performance Comparison', loc='left', pad=10)
    ...
    def _plot_explainability_comparison(self, ax):
        ...
        composite = [
            (p + i + t) / 3
            for p, i, t in zip(provenance, interpretability, traceability)
        ]
    ...
    def fig9_epidemiological_fidelity(self, archetypes, epidemiology):
        ...
        ax1.set_title(
            '(A) Diagnosis Distribution: Expected vs. Observed', loc='left', pad=10
        )
    ...
    def _plot_diagnosis_distribution(self, ax, archetypes, epidemiology):
        ...
        verdict = "PASS" if validation["accept"] else "FAIL"
        textstr += f"Result: {verdict} (α=0.05)"
    ...
    def _plot_age_distribution(self, ax, archetypes, epidemiology):
        ...
        for a in archetypes:
            data.append(
                {'diagnosis': a.diagnosis, 'age': a.parameters.get('age', np.nan)}
            )
    ...
    def _plot_chisquared_test(self, ax, archetypes, epidemiology):
        ...
        # Color by magnitude
        colors = [
            '#e74c3c' if c > 1 else '#f39c12' if c > 0.5 else '#2ecc71'
    ...
    def fig10_critical_coverage(self, archetypes, param_space):
        ...
        ax4.set_title('(D) Critical Scenario Coverage Heatmap', loc='left', pad=10)
    ...
    def _plot_critical_breakdown(self, ax, archetypes):
        ...
        # Match rate
        match_rate = (
            1 - abs(critical_count - expected_critical) / expected_critical
        ) * 100
        deviation = abs(critical_count - expected_critical) / expected_critical * 100
    ...
    def _plot_coverage_heatmap(self, ax, archetypes):
        ...
        # Create heatmap
        im = ax.imshow(coverage_matrix, cmap='RdYlGn', aspect='auto', vmin=60, vmax=100)
