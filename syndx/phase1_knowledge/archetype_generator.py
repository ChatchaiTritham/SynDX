...
class ArchetypeGenerator:
    ...
    def generate_archetypes(
            self,
            n_target: int = 8400) -> List[ClinicalArchetype]:
        """
    ...
    def _sample_symptom_duration(
            ...
            diagnosis: DiagnosisCategory) -> float:
    ...
    def _generate_feature_vector(self, timing, trigger, diagnosis, age, gender,
        ...
        # Symptoms (30)
        features[idx] = np.log1p(symptom_duration) / \
            10.0  # Log-normalized duration
        ...
        features[idx + 1] = symptom_severity / \
            10.0 * float(nausea)  # Severity × nausea
        idx += 10
...
if __name__ == "__main__":
    ...
    for dx, count in sorted(
            stats['diagnosis_distribution'].items(), key=lambda x: -x[1])[:5]:
        print(f" {dx}: {count}")
