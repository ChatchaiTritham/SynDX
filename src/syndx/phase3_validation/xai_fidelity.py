...
class XAIFidelity:
    ...
    def compute_shap_correlation(self) -> float:
        ...
        arch_arr = np.array(self.archetype_shap)
        synth_arr = np.array(self.synthetic_shap)

        if isinstance(self.archetype_shap, list) or arch_arr.ndim == 3:
            ...
            if isinstance(self.archetype_shap, list):
                ...
                class_pairs = zip(self.archetype_shap, self.synthetic_shap)
            else:
                # Newer SHAP: 3D array (n_samples, n_features, n_classes)
                n_classes = arch_arr.shape[2]
                class_pairs = (
                    (arch_arr[:, :, c], synth_arr[:, :, c])
                    for c in range(n_classes)
                )

            for class_idx, (arch_shap, synth_shap) in enumerate(class_pairs):
                # Flatten SHAP values
            ...
            if correlations:
                ...
                logger.warning("No valid class-wise SHAP correlations were computed")
                avg_correlation = 0.0
    ...
    def _get_global_importance(self, shap_values) -> np.ndarray:
        ...
        if isinstance(shap_values, list):
            ...
            # Average absolute SHAP across classes (axis=2), then samples (axis=0)
            abs_shap = np.mean(np.abs(shap_arr), axis=2)
        else:
            # Binary case
    ...
    def compute_interaction_fidelity(
        self,
        arch_interactions: Optional[np.ndarray] = None,
        synth_interactions: Optional[np.ndarray] = None,
    ) -> float:
        """
        ...
        diff_norm = np.linalg.norm(arch_interactions - synth_interactions, ord='fro')
    ...
    def compute_all_metrics(
        ...
        if arch_interactions is not None and synth_interactions is not None:
            self.compute_interaction_fidelity(arch_interactions, synth_interactions)
    ...
    def summary(self) -> Dict:
        ...
        if overall >= 0.85:
            interpretation = (
                "Excellent - Synthetic data preserves explanations very well"
            )
        elif overall >= 0.70:
            interpretation = (
                "Good - Synthetic data maintains most explainability properties"
            )
        elif overall >= 0.50:
        ...
        return {
            ...
            'recommendation': (
                'Use synthetic data for XAI studies'
                if overall >= 0.70
                else 'Caution advised for XAI applications'
            ),
        }
