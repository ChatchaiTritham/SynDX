...
class WeightedEnsembleMerger:
    ...
    def merge_datasets(self, datasets: List[pd.DataFrame]) -> pd.DataFrame:
        ...
        for i, ds in enumerate(datasets):
            if len(ds) < target_size:
                ...
                upsampled = ds.sample(n=target_size, replace=True, random_state=42)
                resampled_datasets.append(upsampled.reset_index(drop=True))
            else:
                # Downsample if too large
                if len(ds) > target_size:
                    downsampled = ds.sample(n=target_size, random_state=42)
                    resampled_datasets.append(downsampled.reset_index(drop=True))
                else:
        ...
        for col in common_cols:
            ...
            if all(pd.api.types.is_numeric_dtype(ds[col]) for ds in resampled_datasets):
                ...
                for i, ds in enumerate(resampled_datasets):
                    ...
                    weighted_counts = {
                        k: v * self.weights[i] for k, v in value_counts.items()
                    }
    ...
    def _apply_diversity_sampling(
        self, df: pd.DataFrame, n_clusters: int = 50
    ) -> pd.DataFrame:
        """
        ...
        """
        logger.info(f"Applying diversity-aware sampling with {n_clusters} clusters...")
        ...
        if n_clusters < 2:
            logger.warning("Too few samples for clustering, returning original data")
            return df
        ...
        try:
            ...
            cluster_series = pd.Series(cluster_labels, index=cluster_data.index)
            df_copy = df.copy()
            df_copy = df_copy.join(cluster_series.rename('cluster_label'), how='left')
            ...
            cluster_counts = df_copy['cluster_label'].value_counts().sort_index()
            ...
            if total_target > 0:
                for cluster_id in target_samples_per_cluster:
                    target_samples_per_cluster[cluster_id] = int(
                        target_samples_per_cluster[cluster_id]
                        * total_samples
                        / total_target
                    )
                ...
                if cluster_id == -1:  # Handle NaN cluster separately
                    ...
                    if len(nan_cluster_data) > 0:
                        ...
                        sampled_nan = nan_cluster_data.sample(
                            n=min(len(nan_cluster_data), total_samples // n_clusters),
                            replace=True,
                            random_state=42,
                        )
                        sampled_dfs.append(sampled_nan)
                else:
                    cluster_data_subset = df_copy[
                        df_copy['cluster_label'] == cluster_id
                    ]
    ...
    def get_merge_statistics(
        self, original_datasets: List[pd.DataFrame], merged_dataset: pd.DataFrame
    ) -> Dict[str, any]:
        """
...
class DiversityAwareSampler:
    ...
    def __init__(self, n_clusters: int = 50, random_state: int = 42):
        ...
        logger.info(f"Initialized DiversityAwareSampler with {n_clusters} clusters")

    def sample_diverse_subset(self, df: pd.DataFrame, n_samples: int) -> pd.DataFrame:
        """
        ...
        if len(cluster_data) < 2:
            logger.warning("Too few samples for clustering, using random sampling")
            return df.sample(n=min(n_samples, len(df)), random_state=self.random_state)
        ...
        try:
            ...
            # Fill NaN cluster labels (for rows that had NaN in numeric cols)
            # with -1
            df_with_clusters['cluster_label'] = df_with_clusters[
                'cluster_label'
            ].fillna(-1)

            # Calculate cluster sizes
            cluster_counts = (
                df_with_clusters['cluster_label'].value_counts().sort_index()
            )
            ...
            for cluster_id, weight in cluster_weights.items():
                ...
                cluster_data_subset = df_with_clusters[
                    df_with_clusters['cluster_label'] == cluster_id
                ]
                ...
                if len(cluster_data_subset) > 0:
                    if n_cluster_samples <= len(cluster_data_subset):
                        ...
                        additional_needed = n_cluster_samples - len(cluster_data_subset)
            ...
            if samples_per_cluster:
                ...
                if len(diverse_subset) > n_samples:
                    diverse_subset = diverse_subset.sample(
                        n=n_samples, random_state=self.random_state
                    ).reset_index(drop=True)
                ...
                return df.sample(
                    n=min(n_samples, len(df)), random_state=self.random_state
                )

        except Exception as e:
            logger.warning(f"Diversity sampling failed: {e}, using random sampling")
            return df.sample(n=min(n_samples, len(df)), random_state=self.random_state)
...
class EnsembleIntegrationPipeline:
    ...
    def __init__(
        self,
        ensemble_weights: List[float] = [0.25, 0.20, 0.25, 0.15, 0.15],
    ...
    def integrate_layers(
        self, layer_outputs: List[pd.DataFrame], target_size: Optional[int] = None
    ) -> pd.DataFrame:
        """
    ...
    def get_integration_report(
        self, original_datasets: List[pd.DataFrame], integrated_dataset: pd.DataFrame
    ) -> Dict[str, Any]:
        """
        ...
        report['data_completeness_ratio'] = (
            1
            - (
                report['integrated_missing_values']
                / (len(integrated_dataset) * integrated_features)
            )
            if integrated_features > 0
            else 1.0
        )
...
if __name__ == '__main__':
    ...
    layer2_data['bp_diastolic'] = layer2_data['bp_systolic'] * 0.65 + np.random.normal(
        0, 10, n_samples
    )
    ...
    layer3_data = pd.DataFrame(
        {
            ...
            'chief_complaint': np.random.choice(
                ['dizziness', 'vertigo', 'lightheadedness'], n_samples
            ),
            'onset_pattern': np.random.choice(
                ['acute', 'episodic', 'chronic'], n_samples
            ),
            'diagnosis': np.random.choice(
                ['stroke', 'bppv', 'vn', 'menieres', 'migraine'], n_samples
            ),
    ...
    layer5_data = pd.DataFrame(
        {
            ...
            'validation_passed': np.random.choice(
                [True, False], n_samples, p=[0.97, 0.03]
            ),
            'ti_trate_compliant': np.random.choice(
                [True, False], n_samples, p=[0.98, 0.02]
            ),
