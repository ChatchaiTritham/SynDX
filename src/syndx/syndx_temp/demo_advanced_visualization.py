...
def main():
                ...
                for i in range(self.n_archetypes):
                    archetype = {
                        ...
                        'trigger_type': np.random.choice(
                            ['spontaneous', 'positional', 'head_movement']
                        ),
                        'diagnosis': np.random.choice(
                            ['stroke', 'bppv', 'vn', 'menieres']
                        ),
        ...
        class MockBayesianNetworkGenerator:
            ...
            def generate_samples(self, n_patients):
                ...
                data = {
                    ...
                    'atrial_fibrillation': np.random.choice(
                        [0, 1], n_patients, p=[0.95, 0.05]
                    ),
                ...
                df['bp_diastolic'] = df['bp_systolic'] * 0.65 + np.random.normal(
                    0, 10, n_patients
                )
            ...
            def generate_samples(self, n_patients):
                ...
                data = {
                    ...
                    'chief_complaint': np.random.choice(
                        ['dizziness', 'vertigo', 'lightheadedness'], n_patients
                    ),
                    'onset_pattern': np.random.choice(
                        ['acute', 'episodic', 'chronic'], n_patients
                    ),
                    'trigger': np.random.choice(
                        ['spontaneous', 'positional', 'head_movement'], n_patients
                    ),
                    'diagnosis': np.random.choice(
                        ['stroke', 'bppv', 'vn', 'menieres', 'migraine'], n_patients
                    ),
                    'confidence': np.random.uniform(0.7, 1.0, n_patients),
                    'urgency': np.random.choice(
                        [0, 1, 2], n_patients, p=[0.6, 0.3, 0.1]
                    ),
                }
        ...
        class MockSynDXHybridPipeline:
            ...
            def run_full_pipeline(self, n_patients=10000):
                ...
                validated_rules = self.layer5_counterfactual.validate_samples(
                    rules_with_provenance, validation_type="ti_trate_consistency"
                )
            ...
            def get_statistics(self):
                ...
                stats = {
                    ...
                    'layer_statistics': {
                        'combinatorial': len(
                            self.layer_outputs.get('combinatorial', [])
                        ),
                        'bayesian': len(self.layer_outputs.get('bayesian', [])),
                        'rules': len(self.layer_outputs.get('rules', [])),
                        'rules_provenance': len(
                            self.layer_outputs.get('rules_provenance', [])
                        ),
                        'rules_validated': len(
                            self.layer_outputs.get('rules_validated', [])
                        ),
                        'final_dataset': (
                            len(self.layer_outputs.get('final_dataset', []))
                            if 'final_dataset' in self.layer_outputs
                            else 0
                        ),
                    },
                }
        ...
        class MockSynDXDatasetGenerator:
            ...
            def get_statistics(self):
                stats = {
                    ...
                    'total_features': sum(
                        len(df.columns) for df in self.datasets.values()
                    ),
                    'datasets': {
                        name: {'samples': len(df), 'features': len(df.columns)}
                        for name, df in self.datasets.items()
                    },
            ...
            def generate_all_visualizations(self, datasets):
                ...
                for layer_name in [
                    ...
                    if layer_name in datasets:
                        ...
                        ax.set_ylim(0, 1)
                        ax.set_title(
                            f'{layer_name.title()} Visualization',
                            fontsize=12,
                            fontweight='bold',
                        )
                ...
                # Add title
                ax.set_title(
                    'SynDX-Hybrid Five-Layer Architecture',
                    fontsize=16,
                    fontweight='bold',
                    pad=20,
                )
    ...
    for i, col in enumerate(key_features):
        if i < len(axes.flat):
            ...
            axes.flat[i].set_title(
                f'Distribution of {col}', fontsize=10, fontweight='bold'
            )
    ...
    plt.suptitle(
        'Key Feature Distributions in Ensemble Dataset', fontsize=14, fontweight='bold'
    )
    plt.tight_layout()
    ...
    clinical_cols = [
        col
        for col in generator.datasets['ensemble'].columns
        if any(
            keyword in col.lower()
            for keyword in ['severity', 'confidence', 'risk', 'urgency']
        )
    ][:3]
    ...
    for i, col in enumerate(clinical_cols):
        if i < len(axes):
            axes[i].scatter(
                generator.datasets['ensemble']['age'],
            ...
            axes[i].plot(
                generator.datasets['ensemble']['age'],
                p(generator.datasets['ensemble']['age']),
                "r--",
                alpha=0.8,
            )
    ...
    plt.suptitle(
        'Clinical Indicators vs Age Relationships', fontsize=14, fontweight='bold'
    )
    plt.tight_layout()
    age_corr_fig_path = (
        visualizer.figure_dir / 'performance_age_clinical_correlations.png'
    )
    ...
    print(
        f"  2. Bayesian Networks: Probabilistic dependencies from epidemiological data"
    )
    print(
        f"  3. Rule-Based Expert Systems: Clinical guidelines as formal IF-THEN rules"
    )
