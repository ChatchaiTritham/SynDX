...
class PerturbationEngine:
    ...
    def validate_samples(
        self, data: pd.DataFrame, validation_type: str = "ti_trate_consistency"
    ) -> pd.DataFrame:
        """
    ...
    def _validate_ti_trate_consistency(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for idx in df.index:
            ...
            # factors = stroke
            acute_spontaneous = (
                str(row.get('timing_pattern', '')).lower() == 'acute'
                and str(row.get('trigger_type', '')).lower() == 'spontaneous'
            )

            central_hints = row.get('hints_central', False) or (
                row.get('hit_abnormal', False)
                and row.get('nystagmus_direction_changing', False)
            )

            vascular_risk = (
                row.get('cvd_risk', False)
                or row.get('hypertension', False)
                or row.get('diabetes', False)
            )

            age_factor = row.get('age', 0) >= 50

            expected_stroke = (
                acute_spontaneous and central_hints and vascular_risk and age_factor
            )
            ...
            if expected_stroke and not is_stroke:
                ...
                df.at[idx, 'validation_notes'] = (
                    "TiTrATE violation: acute spontaneous with central HINTS and risk factors should suggest stroke"
                )
                ...
                if not self._is_stroke_justified(row):
                    df.at[idx, 'validation_ti_trate_consistent'] = False
                    df.at[idx, 'validation_notes'] = (
                        "TiTrATE violation: stroke diagnosis without appropriate indicators"
                    )
    ...
    def _validate_age_perturbation(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for idx in df.index:
            ...
            if 'stroke' in original_diagnosis and original_age > 60:
                ...
                df.at[idx, 'validation_notes'] = (
                    f"Age perturbation: {original_age}→{perturbed_age}, diagnosis should reconsider stroke likelihood"
                )

        return df

    def _validate_risk_factor_perturbation(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        ...
        for idx in df.index:
            ...
            original_hypertension = (
                df.at[idx, 'hypertension'] if 'hypertension' in df.columns else False
            )
            original_diabetes = (
                df.at[idx, 'diabetes'] if 'diabetes' in df.columns else False
            )
            original_age = df.at[idx, 'age'] if 'age' in df.columns else 50
            ...
            if ('stroke' in original_diagnosis or 'serious' in original_diagnosis) and (
                ...
                df.at[idx, 'validation_notes'] = (
                    "Risk factor perturbation would likely change diagnosis"
                )
    ...
    def generate_counterfactuals(
        self, data: pd.DataFrame, n_perturbations: int = 100
    ) -> pd.DataFrame:
        """
        ...
        """
        logger.info(f"Generating {n_perturbations} counterfactual perturbations...")
    ...
    def get_validation_report(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if 'validation_ti_trate_consistent' in df.columns:
            report['ti_trate_consistent_count'] = df[
                'validation_ti_trate_consistent'
            ].sum()
            report['ti_trate_inconsistent_count'] = (
                len(df) - report['ti_trate_consistent_count']
            )
            report['ti_trate_consistency_rate'] = report[
                'ti_trate_consistent_count'
            ] / len(df)
        else:
        ...
        if 'validation_age_perturbation_consistent' in df.columns:
            report['age_perturbation_consistent_count'] = df[
                'validation_age_perturbation_consistent'
            ].sum()
            report['age_perturbation_consistency_rate'] = report[
                'age_perturbation_consistent_count'
            ] / len(df)
        else:
        ...
        if 'validation_notes' in df.columns:
            report['validation_note_types'] = (
                df['validation_notes'].value_counts().to_dict()
            )
            report['samples_with_issues'] = (df['validation_notes'] != "").sum()
        else:
...
class TiTrATEConsistencyChecker:
    ...
    def validate_pathway(self, patient_data: pd.Series) -> Dict[str, Any]:
        ...
        if (
            ...
            and age >= 50
            and (hypertension or diabetes or atrial_fibrillation)
        ):

            # Check HINTS components
            central_hints = (
                hit_abnormal or nystagmus_direction_changing or skew_deviation
            )
            ...
            if central_hints and 'stroke' not in results['original_diagnosis'].lower():
                ...
                not central_hints and 'stroke' in results['original_diagnosis'].lower()
            ):
    ...
    def validate_dataset(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df_with_validation['validation_issues'] = validation_df[
            'validation_issues'
        ].apply(lambda x: '; '.join(x) if x else '')
        df_with_validation['confidence_adjustment'] = validation_df[
...
class ValidationEngine:
    ...
    def validate_synthetic_data(
        ...
        validation_types: List[str] = ['ti_trate', 'statistical', 'clinical'],
        ...
        for validation_type in validation_types:
            if validation_type == 'ti_trate':
                ...
                validated_data = self.titrate_checker.validate_dataset(validated_data)
                ...
                results['validation_results']['ti_trate'] = {
                    ...
                    'compliant_samples': int(
                        validated_data['ti_trate_compliant'].sum()
                    ),
                    'non_compliant_samples': len(synthetic_data)
                    - int(validated_data['ti_trate_compliant'].sum()),
                }

            elif validation_type == 'statistical':
                # Statistical realism validation
                results['validation_results']['statistical'] = (
                    self._validate_statistical_realism(validated_data)
                )

            elif validation_type == 'clinical':
                # Clinical plausibility validation
                results['validation_results']['clinical'] = (
                    self._validate_clinical_plausibility(validated_data)
                )
        ...
        ti_trate_score = (
            ...
            .get('ti_trate', {})
            .get('compliance_rate', 1.0)
        )
        stat_score = (
            results['validation_results']
            .get('statistical', {})
            .get('realism_score', 1.0)
        )
        clinical_score = (
            results['validation_results']
            .get('clinical', {})
            .get('plausibility_rate', 1.0)
        )

        results['overall_validation_score'] = (
            ti_trate_score + stat_score + clinical_score
        ) / 3
    ...
    def _validate_statistical_realism(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if len(numeric_cols) > 0:
            ...
            for col in numeric_cols[:10]:
                ...
                # example)
                ref_dist = np.random.normal(df[col].mean(), df[col].std(), len(df))
                # Calculate JS divergence (simplified)
                hist1, _ = np.histogram(df[col].dropna(), bins=20, density=True)
                ...
                m = 0.5 * (hist1 + hist2)
                js_div = 0.5 * (
                    np.sum(hist1 * np.log(hist1 / m))
                    + np.sum(hist2 * np.log(hist2 / m))
                )
                js_divergence += js_div

            js_divergence = (
                js_divergence / min(10, len(numeric_cols)) if numeric_cols else 0.0
            )
        else:
    ...
    def _validate_clinical_plausibility(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        for idx in df.index:
            ...
            if age < 30 and (hypertension or diabetes) and 'stroke' in diagnosis:
                implausible_count += 1
            # Example: very elderly (age > 90) with benign diagnosis but severe
            # symptoms
            elif (
                age > 90 and 'benign' in diagnosis and row.get('severity_score', 5) > 7
            ):
                implausible_count += 1

        plausibility_rate = (
            1 - (implausible_count / total_checks) if total_checks > 0 else 1.0
        )
...
if __name__ == '__main__':
    ...
    sample_data = pd.DataFrame(
        {
            ...
            'timing_pattern': np.random.choice(
                ['acute', 'episodic', 'chronic'], 100, p=[0.3, 0.5, 0.2]
            ),
            'trigger_type': np.random.choice(
                ['spontaneous', 'positional', 'head_movement'], 100, p=[0.4, 0.4, 0.2]
            ),
            'hit_abnormal': np.random.choice([0, 1], 100, p=[0.7, 0.3]),
            'nystagmus_direction_changing': np.random.choice(
                [0, 1], 100, p=[0.85, 0.15]
            ),
            ...
            'diagnosis': np.random.choice(
                ['stroke', 'bppv', 'vn', 'menieres', 'migraine'],
                100,
    ...
    validated_data = perturbation_engine.validate_samples(
        sample_data, validation_type="ti_trate_consistency"
    )
    ...
    validation_results, final_data = validation_engine.validate_synthetic_data(
        ti_trate_validated, validation_types=['ti_trate', 'statistical', 'clinical']
    )
