...
class SynDXVisualizer:
    ...
    def __init__(self, dataset_generator, output_dir: str = 'figures'):
        ...
        logger.info(f"Initialized SynDXVisualizer with output directory: {self.figure_dir}")
    ...
    def create_layer1_visualizations(self, data: pd.DataFrame):
        ...
        if 'timing_pattern' in data.columns:
            ...
            ax.set_xticklabels(
                [t.replace('_', ' ').title() for t in timing_counts.index],
                rotation=45,
                ha='right',
            )
        ...
        if 'age' in data.columns and 'diagnosis' in data.columns:
            ...
            diagnosis_counts = (
                data['diagnosis'].value_counts().head(10)
            )  # Top 10 diagnoses
            filtered_data = data[data['diagnosis'].isin(diagnosis_counts.index)]

            sns.boxplot(data=filtered_data, x='diagnosis', y='age', ax=ax)
            ax.set_xlabel('Diagnosis Category')
            ax.set_ylabel('Age')
            ax.set_title(
                'Age Distribution by Diagnosis Category (Layer 1)',
                fontsize=12,
                fontweight='bold',
            )
        ...
        if 'trigger_type' in data.columns:
            ...
            ax.set_ylabel('Count')
            ax.set_title(
                'Distribution of TiTrATE Trigger Types', fontsize=12, fontweight='bold'
            )
            ax.set_xticks(range(len(trigger_counts)))
            ax.set_xticklabels(
                [t.replace('_', ' ').title() for t in trigger_counts.index],
                rotation=45,
                ha='right',
                fontsize=8,
            )
    ...
    def create_layer3_visualizations(self, data: pd.DataFrame):
        ...
        if 'diagnosis' in data.columns:
            fig, ax = plt.subplots(figsize=(12, 8))
            diag_counts = data['diagnosis'].value_counts().head(15)  # Top 15 diagnoses
            ...
            ax.set_xticklabels(
                [d.replace('_', ' ').title() for d in diag_counts.index],
                rotation=45,
                ha='right',
                fontsize=8,
            )
    ...
    def create_layer4_visualizations(self, data: pd.DataFrame):
        ...
        prov_cols = [
            ...
            if 'provenance' in col.lower() or 'source' in col.lower()
        ]
        if prov_cols:
            ...
            if source_counts:
                ...
                ax.set_xticklabels(
                    [s.replace('_', ' ').title() for s in sources],
                    rotation=45,
                    ha='right',
                    fontsize=8,
                )
    ...
    def create_layer5_visualizations(self, data: pd.DataFrame):
        ...
        val_cols = [
            col
            for col in data.columns
            if 'validation' in col.lower() or 'consistent' in col.lower()
        ]
        ...
        if val_cols:
            ...
            if val_results:
                ...
                ax.set_ylim(0, 1)
                ax.set_title(
                    'Validation Status (Layer 5 - Counterfactual)',
                    fontsize=12,
                    fontweight='bold',
                )
        else:
    ...
    def create_ensemble_visualizations(self, data: pd.DataFrame):
        ...
        layers = ['Layer 1', 'Layer 2', 'Layer 3', 'Layer 4', 'Layer 5', 'Ensemble']
        ...
        bars = ax.bar(
            layers,
            feature_counts,
            color=['skyblue', 'lightgreen', 'orange', 'pink', 'lightgray', 'gold'],
            edgecolor='black',
            linewidth=0.5,
        )
        ax.set_xlabel('Layer')
        ...
        ax.set_title(
            'Feature Count Comparison Across SynDX-Hybrid Layers',
            fontsize=12,
            fontweight='bold',
        )
        ...
        if len(numeric_cols) > 1:
            ...
            plt.suptitle(
                'Key Variable Distributions (Ensemble)', fontsize=14, fontweight='bold'
            )
            plt.tight_layout()
    ...
    def create_comprehensive_visualizations(self, datasets: Dict[str, pd.DataFrame]):
        """
        ...
        for i, (x, y, layer) in enumerate(zip(x_pos, y_pos, layers)):
            ...
            ax.add_patch(rect)
            ax.text(
                x, y, layer, ha='center', va='center', fontweight='bold', fontsize=9
            )
