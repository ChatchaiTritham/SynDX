...
class ArchetypeGenerator:
    ...
    def generate_archetypes(self) -> List[ClinicalArchetype]:
        ...
        for i in range(min(self.n_archetypes, 1000)):  # Limit for demo
            ...
            diagnosis_idx = (i // (len(timing_patterns) * len(trigger_types))) % len(
                diagnoses
            )
    ...
    def _generate_feature_vector(
        ...
        # Comorbidities (10-29)
        has_hypertension = (age > 50 and np.random.rand() < 0.4) or (
            age > 70 and np.random.rand() < 0.7
        )
        has_diabetes = (age > 40 and np.random.rand() < 0.15) or (
            age > 60 and np.random.rand() < 0.25
        )
        has_cvd = age > 60 and has_hypertension and np.random.rand() < 0.3
        has_migraine = (age < 60 and np.random.rand() < 0.3) or (
        ...
        if diagnosis == DiagnosisCategory.STROKE:
            ...
            features[61] = np.random.choice(
                [0, 1], p=[0.8, 0.2]
            )  # Abnormal HIT (peripheral)
            features[62] = np.random.choice(
                [0, 1], p=[0.9, 0.1]
            )  # Abnormal HIT (central)
            features[63] = np.random.choice([0, 1], p=[0.7, 0.3])  # No nystagmus
            features[64] = np.random.choice(
                [0, 1], p=[0.8, 0.2]
            )  # Peripheral nystagmus
            features[65] = np.random.choice([0, 1], p=[0.9, 0.1])  # Central nystagmus
            # Direction-changing nystagmus
            features[66] = np.random.choice([0, 1], p=[0.9, 0.1])
            features[67] = np.random.choice([0, 1], p=[0.95, 0.05])  # Skew deviation
        ...
        features[121] = min(bp_diastolic / 140.0, 1.0)  # Normalized diastolic BP
        features[122] = min(max(heart_rate, 40) / 180.0, 1.0)  # Normalized heart rate

        # Derived features (140-149) - interaction terms
        features[140] = features[0] * features[10]  # Age × Hypertension
        features[141] = features[31] * features[67]  # Severity × Skew deviation
...
if __name__ == '__main__':
    ...
    generator = ArchetypeGenerator(n_archetypes=100, random_seed=42)  # Smaller for demo
    ...
    print(
        df.head()[
            ['timing_pattern', 'trigger_type', 'diagnosis', 'age', 'gender', 'urgency']
        ].to_string()
    )
