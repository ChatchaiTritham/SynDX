...
class BayesianNetworkGenerator:
    ...
    def __init__(self, n_nodes: int = 45, random_seed: int = 42):
        ...
        logger.info(f"Initialized BayesianNetworkGenerator with {n_nodes} nodes")

    def _define_nodes(self) -> List[str]:
        """Define the nodes in the Bayesian network based on clinical variables."""
        # Root nodes (no parents)
        root_nodes = ['age', 'sex', 'race', 'family_history']
    ...
    def _define_conditional_probability_tables(self) -> Dict[str, np.ndarray]:
        ...
        if 'hypertension' in self.nodes and 'age' in self.nodes:
            ...
            hypertension_given_age = np.zeros(
                (2, n_age_bins)
            )  # [no_htn, htn] x [age_bins]

            for age_bin in range(n_age_bins):
                # Higher probability of hypertension with age
                htn_prob = min(0.1 + age_bin * 0.12, 0.8)  # Up to 80% probability
                hypertension_given_age[0, age_bin] = 1 - htn_prob  # No hypertension
                # Has hypertension
        ...
        if (
            'diabetes' in self.nodes
            and 'age' in self.nodes
            and 'hypertension' in self.nodes
        ):
            ...
            for age_bin in range(n_age_bins):
                for htn_status in range(2):
                    ...
                    diabetes_given_age_htn[0, htn_status, age_bin] = 1 - dm_prob
        ...
        for node in self.nodes:
            if node not in cpts:
                ...
                cpts[node] = np.random.dirichlet(
                    np.ones(n_states), size=max(1, self.n_nodes // 10)
                )
    ...
    def generate_samples(self, n_patients: int = 10000) -> pd.DataFrame:
        ...
        """
        logger.info(f"Generating {n_patients} samples using Bayesian network...")
        ...
        for i in range(n_patients):
            ...
            atrial_fibrillation = self._sample_atrial_fibrillation(age, hypertension)
            migraine_history = self._sample_migraine(age, sex)

            # Generate symptom characteristics based on comorbidities
            timing_pattern = self._sample_timing_pattern(age, hypertension, diabetes)
            trigger_type = self._sample_trigger_type(timing_pattern, migraine_history)
    ...
    def _sample_atrial_fibrillation(self, age: int, hypertension: bool) -> bool:
    ...
    def _sample_trigger_type(self, timing_pattern: str, migraine_history: bool) -> str:
    ...
    def _determine_diagnosis(
        ...
        if (
            timing == 'acute'
            and trigger == 'spontaneous'
            and exam['hit_abnormal']
            and exam['nystagmus_direction_changing']
            and exam['skew_deviation']
            and age >= 50
            and (htn or dm or af)
        ):
            ...
            and exam['dix_hallpike_positive'].startswith('positive')
        ):
            return 'bppv_posterior_canal'
        elif (
            timing == 'acute'
            and trigger == 'spontaneous'
            and not exam['hit_abnormal']
            and exam['nystagmus_type'] == 'peripheral'
            and not exam['skew_deviation']
        ):
            return 'vestibular_neuritis'
        elif migraine and timing == 'episodic' and trigger in ['spontaneous', 'stress']:
