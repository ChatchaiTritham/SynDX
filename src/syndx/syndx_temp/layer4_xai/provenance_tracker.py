...
class ProvenanceTracker:
    ...
    def add_provenance(
        ...
        # provenance columns)
        feature_cols = [col for col in df.columns if not col.startswith('provenance_')]
    ...
    def _calculate_pti(self, df: pd.DataFrame) -> float:
        ...
        for feature in original_features:
            ...
            has_prov = any(f"{feature}_provenance_" in col for col in df.columns)
            if has_prov:
    ...
    def validate_provenance(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        results['features_with_provenance'] = int(pti * results['total_features'])
    ...
    def get_provenance_report(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if 'provenance_source_layer' in df.columns:
            report['source_layer_distribution'] = (
...
class ExplanationGenerator:
    ...
    def _add_shap_explanations(
        self, df: pd.DataFrame, predictions: Optional[np.ndarray]
    ) -> pd.DataFrame:
        ...
        for i, col in enumerate(
            numeric_cols[:20]
        ):  # Add SHAP for first 20 numeric features
            df[f'shap_importance_{col}'] = np.random.uniform(-1, 1, len(df))

        return df

    def _add_lime_explanations(
        self, df: pd.DataFrame, predictions: Optional[np.ndarray]
    ) -> pd.DataFrame:
        ...
        feature_cols = [
            col for col in df.columns if col not in ['patient_id', 'provenance_*']
        ]

        for i, col in enumerate(feature_cols[:10]):  # Add LIME for first 10 features
            df[f'lime_contribution_{col}'] = np.random.uniform(-0.5, 0.5, len(df))
    ...
    def _add_counterfactual_explanations(self, df: pd.DataFrame) -> pd.DataFrame:
...
class FeatureAttributionAnalyzer:
    ...
    def analyze_feature_importance(
        self, df: pd.DataFrame, target_column: str = 'diagnosis'
    ) -> Dict[str, Any]:
        """
        ...
        if target_column in df.columns and len(categorical_cols) > 0:
            ...
            sorted_categorical = dict(
                sorted(chi_square_tests.items(), key=lambda x: x[1], reverse=True)
            )
            analysis['categorical_associations'] = sorted_categorical
            analysis['top_10_categorical'] = dict(list(sorted_categorical.items())[:10])

        # Calculate feature importance statistics
        analysis['total_features_analyzed'] = len(numeric_cols) + len(categorical_cols)
...
if __name__ == '__main__':
    ...
    sample_data = pd.DataFrame(
        {
            ...
            'trigger_type': np.random.choice(
                ['spontaneous', 'positional', 'head_movement'], 100
            ),
            'diagnosis': np.random.choice(
                ['stroke', 'bppv', 'vn', 'menieres', 'migraine'], 100
            ),
    ...
    prov_cols = [
        col for col in data_with_provenance.columns if 'provenance' in col.lower()
    ][:5]
    print(data_with_provenance[['patient_id', 'diagnosis'] + prov_cols[:3]].head())
    ...
    explained_data = explanation_gen.generate_explanations(
        data_with_provenance, method='combined'
    )
    ...
    importance_analysis = feature_analyzer.analyze_feature_importance(
        explained_data, target_column='diagnosis'
    )
