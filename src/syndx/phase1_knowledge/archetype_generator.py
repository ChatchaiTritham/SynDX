...
from .titrate_formalizer import (ClinicalArchetype, DiagnosisCategory,
                                 ExaminationFindings, TimingPattern,
                                 TiTrATEFormalizer, TriggerType)
...
class ArchetypeGenerator:
    ...
    def _generate_single_archetype(self) -> ClinicalArchetype:
        ...
        has_cvd = self._sample_cardiovascular_disease(
            age, has_hypertension, has_diabetes
        )
    ...
    def _sample_cardiovascular_disease(
        self, age: int, has_htn: bool, has_dm: bool
    ) -> bool:
    ...
    def _sample_symptom_duration(
        self, timing: TimingPattern, diagnosis: DiagnosisCategory
    ) -> float:
    ...
    def _sample_headache(self, diagnosis: DiagnosisCategory) -> bool:
        ...
        if diagnosis in [
            DiagnosisCategory.MIGRAINE,
            DiagnosisCategory.VESTIBULAR_MIGRAINE,
        ]:
    ...
    def _sample_tinnitus(self, diagnosis: DiagnosisCategory) -> bool:
        ...
        if diagnosis == DiagnosisCategory.MENIERES:
            ...
            DiagnosisCategory.LABYRINTHITIS,
            DiagnosisCategory.VESTIBULAR_NEURITIS,
        ]:
            return np.random.rand() < 0.3
        else:
            return np.random.rand() < 0.15

    def _generate_examination(
        self, diagnosis: DiagnosisCategory, age: int, has_cvd: bool
    ) -> ExaminationFindings:
        ...
        if diagnosis == DiagnosisCategory.BPPV:
            ...
            dix_hallpike = np.random.choice(
                ["negative"] * 8 + ["positive_right", "positive_left"]
            )
    ...
    def _sample_blood_pressure(
        self, age: int, has_cvd: bool, diagnosis: DiagnosisCategory
    ) -> int:
    ...
    def _determine_urgency(
        self, diagnosis: DiagnosisCategory, exam: ExaminationFindings
    ) -> int:
    ...
    def _generate_feature_vector(
        ...
        features[idx + 1] = (
            1.0 if exam.head_impulse_test == "abnormal_peripheral" else 0.0
        )
...
if __name__ == "__main__":
    ...
    for dx, count in sorted(
        stats['diagnosis_distribution'].items(), key=lambda x: -x[1]
    )[:5]:
        print(f" {dx}: {count}")
