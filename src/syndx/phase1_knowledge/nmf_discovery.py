...
class NMFFactorDiscovery:
    ...
    def __init__(
        ...
        if n_components is None:
            ...
            n_components = self._calculate_r_clinical(n_diagnoses, n_parameters)
    ...
    def _calculate_r_clinical(n_diagnoses: int, n_parameters: int) -> int:
        ...
        """
        r = int(np.ceil(np.log2(n_diagnoses) + np.sqrt(n_parameters / 10)))
        logger.info(
    ...
    def _interpret_factors(self, top_k: int = 10) -> List[Dict]:
        ...
        for factor_idx in range(self.n_components):
            ...
            interpretation = {
                ...
                'clinical_pattern': self._infer_clinical_pattern(
    ...
    def _infer_clinical_pattern(self, features: List[str], weights: np.ndarray) -> str:
        """
    ...
    def elbow_analysis(
        self, X: np.ndarray, r_range: range = range(5, 31, 5)
    ) -> Dict[int, float]:
        """
        ...
        for r in r_range:
            model = NMF(n_components=r, init='nndsvd', max_iter=500, random_state=42)
            model.fit(X)
