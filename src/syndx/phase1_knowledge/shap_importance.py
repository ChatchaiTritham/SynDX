...
class SHAPImportanceAnalyzer:
    ...
    def fit(self, X: np.ndarray, y: np.ndarray, feature_names: List[str] = None):
        ...
        if isinstance(shap_values, list):
            ...
            self.shap_values_ = np.mean([np.abs(sv) for sv in shap_values], axis=0)
        else:
    ...
    def _compute_sampling_weights(self):
        ...
        if total_importance > 0:
            ...
            # Uniform if all zero
            self.sampling_weights_ = np.ones_like(self.feature_importance_) / len(
    ...
    def get_sampling_weights(self, feature_indices: List[int] = None) -> np.ndarray:
        """
    ...
    def get_top_features(self, top_k: int = 20) -> List[Tuple[str, float]]:
        ...
        if self.feature_names_:
            features = [
                (self.feature_names_[i], self.feature_importance_[i]) for i in indices
            ]
        else:
            features = [(f"feature_{i}", self.feature_importance_[i]) for i in indices]

        return features

    def get_parameter_importance(
        self, parameter_feature_map: Dict[str, List[int]]
    ) -> Dict[str, float]:
        """
    ...
    def explain_instance(self, x: np.ndarray, feature_names: List[str] = None) -> Dict:
        """
        ...
        names = feature_names or self.feature_names_ or [f"f{i}" for i in range(len(x))]
    ...
    def plot_importance(self, top_k: int = 20, save_path: str = None):
        """
