...
from syndx.core.parameter_space import Archetype, ParameterSpace
...
class XAIGuidedExplorer:
    ...
    def __init__(
        ...
        if enable_profiling:
            ...
            self.profiler = SynDXProfiler(output_dir='outputs/profiling', enabled=True)
        ...
        if n_target is None:
            n_target, breakdown = calculate_n_target(
                ...
                valid_space_size=int(parameter_space.space_size * 0.5),  # Estimate
        ...
        if nmf_factors is None:
            ...
            nmf_factors = NMFFactorDiscovery._calculate_r_clinical(
                parameter_space.D_size, parameter_space.m
            )
    ...
    def explore(self) -> List[Archetype]:
        ...
        # Combine all
        self.final_archetypes = (
            importance_archetypes + critical_archetypes + diversity_archetypes
        )
        ...
        if self.profiler:
            ...
            self.profiler.phase_metrics['Exploration'] = {
                ...
                'subphases': self.profiler.phase_metrics.get('Exploration', {}).get(
                    'subphases', {}
                ),
            }
        ...
        logger.info(f"Match: {len(self.final_archetypes) / self.n_target * 100:.1f}%")
        if self.profiler:
    ...
    def _calculate_initial_sample_size(self) -> int:
        ...
        n0 = int(
            np.ceil(
                np.log(self.param_space.space_size) * np.sqrt(self.param_space.D_size)
            )
        )
    ...
    def _phase2_nmf_discovery(self) -> NMFFactorDiscovery:
        ...
        nmf = NMFFactorDiscovery(
            n_components=self.nmf_factors, random_state=self.random_state
        )
        nmf.fit(X)
    ...
    def _phase3_shap_analysis(self) -> SHAPImportanceAnalyzer:
        ...
        # Fit SHAP analyzer
        shap = SHAPImportanceAnalyzer(model_type='tree', random_state=self.random_state)
        shap.fit(X, y)
    ...
    def _phase4_importance_sampling(self) -> List[Archetype]:
        """Phase 4: Importance-weighted sampling (60%)"""
        logger.info(f"Importance-weighted sampling: {self.n_importance} archetypes...")
    ...
    def _sample_critical_scenario(self, critical_diagnoses: List[str]) -> Archetype:
    ...
    def _phase6_diversity_sampling(
        self, existing_archetypes: List[Archetype]
    ) -> List[Archetype]:
        """Phase 6: Diversity-oriented sampling (10%)"""
        ...
        while len(archetypes) < self.n_diversity and attempts < max_attempts:
            ...
            archetype = self._sample_near_cluster(kmeans.cluster_centers_[cluster_id])
            attempts += 1
    ...
    def _validate_final_set(self):
        ...
        critical_count = sum(
            1 for a in self.final_archetypes if a.diagnosis in ['stroke', 'tia']
        )
    ...
    def get_statistics(self) -> Dict:
        ...
        return {
            ...
            'final_count': len(self.final_archetypes),
