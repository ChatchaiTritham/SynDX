...
class DifferentialPrivacy:
    ...
    def add_noise(self, aggregated_gradient: np.ndarray, batch_size: int) -> np.ndarray:
        """
        ...
        noise = np.random.normal(0, noise_scale, size=aggregated_gradient.shape)
    ...
    def privatize_gradients(
        self, gradients: List[np.ndarray], batch_size: int
    ...
    def get_privacy_spent(
        self, steps: int, batch_size: int, n_samples: int
    ) -> Tuple[float, float]:
        """
        ...
        epsilon_spent = (
            q * np.sqrt(2 * T * np.log(1 / self.delta)) / self.noise_multiplier
        )
    ...
    def check_privacy_budget(self, steps: int, batch_size: int, n_samples: int) -> bool:
        """
        ...
        return bool(eps_spent <= self.epsilon)
    ...
    def get_max_steps(self, batch_size: int, n_samples: int) -> int:
        ...
        # T = (ε · σ / q)² / (2 ln(1/δ))
        max_steps = int((self.epsilon * sigma / q) ** 2 / (2 * np.log(1 / self.delta)))
        ...
        return {
            ...
            'privacy_accounting': {
                ...
                'budget_remaining': max(
                    0, self.epsilon - self.privacy_spent.get('epsilon', 0.0)
                ),
                'within_budget': self.privacy_spent.get('epsilon', 0.0) <= self.epsilon,
            },
        }
...
if __name__ == '__main__':
    ...
    logger.info(f"Generated {batch_size} per-sample gradients (dim={grad_dim})")
