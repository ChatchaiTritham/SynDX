...
class SHAPReweighter:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized SHAPReweighter (model={model_type}, "
            f"background_samples={background_samples})"
        )

    def fit(
        self, X_archetypes: np.ndarray, y_diagnosis: np.ndarray
    ) -> 'SHAPReweighter':
        """
        ...
        if len(X_archetypes) > self.background_samples:
            background_indices = np.random.choice(
                len(X_archetypes), size=self.background_samples, replace=False
            )
        ...
        if isinstance(self.shap_values, list):
            ...
            # Take mean absolute SHAP across classes (axis=2), then samples (axis=0)
    ...
    def get_top_features(self, n: int = 20) -> List[Tuple[int, float]]:
        ...
        return [(int(idx), float(self.feature_weights[idx])) for idx in top_indices]
    ...
    def plot_importance(
        ...
        if feature_names is None:
            feature_names = [f"Feature_{i}" for i in range(len(self.feature_weights))]
        ...
        ax.barh(
            y_pos, weights, color=colors, alpha=0.8, edgecolor='black', linewidth=0.5
        )
        ...
        ax.set_title(
            f'Top {top_n} Features by SHAP Importance', fontsize=12, fontweight='bold'
        )
    ...
    def summary(self) -> Dict:
        ...
        return {
            ...
            'top_feature_weight': float(np.max(self.feature_weights)),
            'min_feature_weight': float(np.min(self.feature_weights)),
            'mean_feature_weight': float(np.mean(self.feature_weights)),
    ...
    def __repr__(self) -> str:
        ...
        if self.feature_weights is None:
            return (
                f"SHAPReweighter(model_type='{self.model_type}', "
                f"background_samples={self.background_samples}, fitted=False)"
            )
        else:
            return (
                f"SHAPReweighter(model_type='{self.model_type}', "
...
def analyze_feature_importance(
    ...
    return {
        ...
        'top_features': [
            {'index': idx, 'name': feature_names[idx], 'importance': weight}
