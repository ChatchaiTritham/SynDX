...
def main():
    ...
    try:
        ...
        class MockBayesianNetworkGenerator:
            ...
            def generate_samples(self, n_patients):
                ...
                data = {
                    ...
                    'bp_diastolic': 120 + np.random.normal(0, 10, n_patients),  # Fixed: calculate properly
                    'stroke_risk_score': np.random.beta(2, 5, n_patients)
                }
                
                df = pd.DataFrame(data)
                # Calculate bp_diastolic after creating the dataframe
                df['bp_diastolic'] = df['bp_systolic'] * 0.65 + np.random.normal(0, 10, n_patients)
        ...
        class MockSynDXHybridPipeline:
            def __init__(self, **kwargs):
                ...
                self.ensemble_weights = kwargs.get('ensemble_weights', [0.25, 0.20, 0.25, 0.15, 0.15])
                self.random_seed = kwargs.get('random_seed', 42)
                
                np.random.seed(self.random_seed)
                
                # Initialize mock layers
                self.layer1_combinatorial = MockArchetypeGenerator(self.n_archetypes, self.random_seed)
                self.layer2_bayesian = MockBayesianNetworkGenerator(self.bayesian_nodes, self.random_seed)
                self.layer3_rules = MockRuleBasedExpertSystem(self.rule_base_size, self.random_seed)
                self.layer4_xai = MockProvenanceTracker()
                self.layer5_counterfactual = MockPerturbationEngine()
                self.ensemble_merger = MockWeightedEnsembleMerger(self.ensemble_weights)
            ...
            def run_full_pipeline(self, n_patients=10000):
                ...
                rules_with_provenance = self.layer4_xai.add_provenance(
                    rule_based_samples,
                    source_layer="rules",
                    source_reference="Clinical Guidelines (AHA/ASA, Bเธฃเธrเธฃเธny ICVD)"
                )
                ...
                combinatorial_processed = self._create_placeholder_dataset(archetypes, "combinatorial")
                bayesian_processed = self._create_placeholder_dataset(bayesian_samples, "bayesian")
                xai_placeholder = self._create_placeholder_dataset(rules_with_provenance, "xai")
                cf_placeholder = self._create_placeholder_dataset(validated_rules, "counterfactual")
            ...
            def _create_placeholder_dataset(self, base_dataset, layer_name):
                ...
                for i in range(5):  # Add 5 layer-specific features
                    df[f'{layer_name}_specific_feature_{i:02d}'] = np.random.random(len(df))
            ...
            def get_statistics(self):
                ...
                stats = {
                    ...
                    'layer_statistics': {
                        'combinatorial': len(self.layer_outputs.get('combinatorial', [])),
                        'bayesian': len(self.layer_outputs.get('bayesian', [])),
                        'rules': len(self.layer_outputs.get('rules', [])),
                        'rules_provenance': len(self.layer_outputs.get('rules_provenance', [])),
                        'rules_validated': len(self.layer_outputs.get('rules_validated', [])),
                        'final_dataset': len(self.layer_outputs.get('final_dataset', [])) if 'final_dataset' in self.layer_outputs else 0
                    }
                }
                return stats
    ...
    if isinstance(synthetic_data, pd.DataFrame) and not synthetic_data.empty:
        ...
        print(synthetic_data.head()[['patient_id', 'age', 'diagnosis', 'confidence', 'urgency']].to_string())
    ...
    try:
        ...
        class MockSynDXVisualizer:
            ...
            def generate_all_visualizations(self, datasets):
                ...
                # Create mock figures
                for layer_name in ['layer1', 'layer2', 'layer3', 'layer4', 'layer5', 'ensemble']:
                    if layer_name in datasets:
                        # Create a simple figure for each layer
                        fig, ax = plt.subplots(figsize=(8, 6))
                        ax.text(0.5, 0.5, f'{layer_name.upper()}\\nVisualization\\n(600 DPI)', 
                               horizontalalignment='center', verticalalignment='center', 
                               transform=ax.transAxes, fontsize=14, fontweight='bold')
                        ax.set_xlim(0, 1)
                        ax.set_ylim(0, 1)
                        ax.set_title(f'{layer_name.title()} Visualization', fontsize=12, fontweight='bold')
                ...
                for i, (x, y, layer) in enumerate(zip(x_pos, y_pos, layers)):
                    ...
                    ax.add_patch(rect)
                    ax.text(x, y, layer, ha='center', va='center', fontweight='bold', fontsize=9)
    ...
    print(f"\\nVisualization generation completed!")
    print(f"Total figures created: {len(list(visualizer.figure_dir.glob('*.png')))}")
    ...
    numeric_cols = generator.datasets['ensemble'].select_dtypes(include=[np.number]).columns.tolist()
    key_features = [col for col in numeric_cols if 'age' in col.lower() or 'severity' in col.lower() or 'confidence' in col.lower()][:4]
    
    for i, col in enumerate(key_features):
        if i < len(axes.flat):
            axes.flat[i].hist(generator.datasets['ensemble'][col].dropna(), bins=50, 
                             color='lightblue', edgecolor='navy', alpha=0.7)
            axes.flat[i].set_title(f'Distribution of {col}', fontsize=10, fontweight='bold')
    ...
    plt.suptitle('Key Feature Distributions in Ensemble Dataset', fontsize=14, fontweight='bold')
    plt.tight_layout()
    perf_fig_path = visualizer.figure_dir / 'performance_key_feature_distributions.png'
    ...
    if len(key_features) >= 2:
        ...
        im = sns.heatmap(corr_matrix, annot=True, fmt='.2f', cmap='RdBu_r', center=0,
                         square=True, ax=ax, cbar_kws={'shrink': 0.8}, mask=mask)
        ax.set_title('Feature Correlation Matrix (Ensemble Dataset)', fontsize=12, fontweight='bold')
        plt.tight_layout()
    ...
    clinical_cols = [col for col in generator.datasets['ensemble'].columns 
                     if any(keyword in col.lower() for keyword in ['severity', 'confidence', 'risk', 'urgency'])][:3]
    ...
    for i, col in enumerate(clinical_cols):
        if i < len(axes):
            ...
            p = np.poly1d(z)
            axes[i].plot(generator.datasets['ensemble']['age'], 
                        p(generator.datasets['ensemble']['age']), "r--", alpha=0.8)
    
    # Hide unused subplots
    for i in range(len(clinical_cols), len(axes)):
        axes[i].set_visible(False)
    
    plt.suptitle('Clinical Indicators vs Age Relationships', fontsize=14, fontweight='bold')
    plt.tight_layout()
    age_corr_fig_path = visualizer.figure_dir / 'performance_age_clinical_correlations.png'
    ...
    print(f"  Total synthetic patients generated: {len(generator.datasets['ensemble']):,}")
